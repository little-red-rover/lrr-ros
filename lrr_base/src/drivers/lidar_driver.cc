...
#define SCAN_POINTS 360
  ...
  LidarDriver::LidarDriver(ros::NodeHandle node_handle,
      ...
      : connection_(this, LIDAR_DATA), filter_chain_("sensor_msgs::LaserScan")
  {
    ...
    filter_chain_.configure("/scan_filter_chain", private_node_handle);
  };
  ...
  void LidarDriver::parse(OutgoingData &data)
  {
    ...
    if (point_batch_.size() >= SUBSAMPLE_POINTS)
    {
      ...
      // std::fill(msg_.ranges.begin(), msg_.ranges.end(), 0.0);
      // std::fill(msg_.intensities.begin(), msg_.intensities.end(), 0.0);
      for (size_t i = 0; i < point_batch_.size(); i++)
      {
        geometry_msgs::Point point = point_batch_[i];
        size_t idx = (int)((atan2(point.y, point.x) / (2 * M_PI)) * SCAN_POINTS +
                           SCAN_POINTS / 2.0);
      ...
      sensor_msgs::LaserScan filtered_msg = sensor_msgs::LaserScan();
      filter_chain_.update(msg_, filtered_msg);
